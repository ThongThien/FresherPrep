...
export function QuizStartPage({ quizId }: { quizId: string }) {
  ...
  useEffect(() => {
    ...
    void fetch("/api/quizzes/" + encodeURIComponent(quizId), { signal: controller.signal })
      .then(async (response) => {
        ...
        if (!(reason instanceof DOMException && reason.name === "AbortError")) setError(t("Unable to load this quiz."));
      });
  ...
  async function start() {
    ...
    } catch { setError(t("Unable to start the quiz. Check your connection and try again.")); }
  ...
  return <div className="mx-auto max-w-3xl">
    <Link className="text-sm font-semibold text-primary" href="/learning-paths">{t("Back to learning")}</Link>
    <header className="mt-6 border-b border-border pb-7">
      <div className="flex flex-wrap gap-2"><Badge variant="info">{t(quiz.language === "EN" ? "English" : "Vietnamese")}</Badge><Badge>{t(quiz.category)}</Badge><Badge>{t(quiz.selectionMode === "FIXED" ? "Fixed questions" : "Rule-based questions")}</Badge></div>
      <h1 className="mt-4 text-3xl font-semibold tracking-tight text-text sm:text-4xl">{quiz.title}</h1>
      <p className="mt-3 text-sm leading-6 text-text-muted">{t("Answer each question once. Your choices are saved by the server and cannot be changed after submission.")}</p>
    </header>
    <section className="mt-8 rounded-lg border border-border bg-surface p-6 shadow-card">
      <h2 className="font-semibold text-text">{t("Before you begin")}</h2>
      <ul className="mt-4 list-disc space-y-2 pl-5 text-sm leading-6 text-text-muted">
        <li>{t("Questions and option order are provided by the assessment.")}</li>
        <li>{t("You can safely reload after the attempt opens.")}</li>
        <li>{t("No time limit is shown because the backend does not provide one.")}</li>
        <li>{t("Passing threshold: {{percent}}%.", { percent: quiz.passPercentage })}</li>
        <li>{t("Assessment score: {{passing}} / {{maximum}} required.", { passing: quiz.passingScore, maximum: quiz.maximumScore })}</li>
        <li>{t("{{count}} questions", { count: quiz.questionCount })}</li>
      </ul>
...
function State({ title, message, onRetry }: { title: string; message: string; onRetry?: () => void }) {
  const { t } = useI18n();
  return <div className="mx-auto max-w-2xl py-10"><h1 className="text-2xl font-semibold text-text">{title}</h1><p className="mt-3 text-sm text-text-muted">{message}</p><div className="mt-5 flex flex-wrap gap-3">{onRetry ? <Button variant="secondary" onClick={onRetry}>{t("Try again")}</Button> : null}<Link className="inline-flex min-h-10 items-center text-sm font-semibold text-primary" href="/learning-paths">{t("Back to learning paths")}</Link></div></div>;
}
