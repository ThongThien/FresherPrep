...
export function QuizResultPage({ quizId, attemptId }: { quizId: string; attemptId: string }) {
  ...
  useEffect(() => {
    ...
    void fetch("/api/quiz-attempts/" + encodeURIComponent(attemptId), { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        ...
        if (value.quizId !== quizId) return setError(t("This result does not belong to the requested quiz."));
        if (value.status !== "SUBMITTED") return router.replace("/quizzes/" + quizId + "/attempts/" + attemptId);
        setAttempt(value);
        setError(undefined);
      }).catch((reason: unknown) => {
        if (!(reason instanceof DOMException && reason.name === "AbortError")) setError(t("Unable to load this result."));
      });
  ...
  async function retryQuiz() {
    ...
    } catch { setError(t("Unable to start another attempt.")); }
    finally { setRetrying(false); }
  }

  if (error === "NOT_FOUND") return <ResultState title={t("Result not found")} message={t("This attempt does not exist or is unavailable to your account.")} />;
  if (!attempt && error) return <ResultState title={t("Result unavailable")} message={error} onRetry={() => setReload((value) => value + 1)} />;
  ...
  return <div className="mx-auto w-full max-w-5xl">
    <header className="border-b border-border pb-8"><div className="flex flex-wrap items-center gap-2"><Badge variant={attempt.passed ? "success" : "danger"}>{attempt.passed ? t("Passed") : t("Not passed")}</Badge><Badge>{t("Submitted")}</Badge></div><h1 className="mt-4 text-3xl font-semibold tracking-tight text-text sm:text-4xl">{attempt.quizTitle}</h1><p className="mt-2 text-sm text-text-muted">{t("Submitted {{date}}", { date: formatDate(attempt.submittedAt, locale) })}</p></header>
    <section className="mt-8 grid gap-4 sm:grid-cols-3" aria-label={t("Result summary")}><Summary label={t("Score")} value={formatAssessmentScore(attempt)} /><Summary label={t("Pass requirement")} value={attempt.passingScore + " / " + attempt.maximumScore + " (" + attempt.passPercentage + "%)"} /><Summary label={t("Answered")} value={answered + " / " + attempt.questions.length} /></section>
    <Progress className="mt-6" value={score} label={t("Final score")} showValue tone={attempt.passed ? "success" : "primary"} />
    {error ? <Feedback className="mt-6" tone="error" title={t("Action failed")}>{error}</Feedback> : null}
    <div className="mt-7 flex flex-wrap gap-3"><Button loading={retrying} onClick={() => void retryQuiz()}>{retrying ? t("Starting...") : t("Try again")}</Button><LinkButton href="/learning-paths">{t("Return to learning paths")}</LinkButton><LinkButton href="/progress#quiz-activity">{t("View quiz history")}</LinkButton></div>
    <section className="mt-12" aria-labelledby="answer-review-title"><h2 id="answer-review-title" className="text-xl font-semibold text-text">{t("Answer review")}</h2><div className="mt-5 space-y-5">{attempt.questions.map((question, index) => <QuestionReview key={question.id} index={index} question={question} />)}</div></section>
  </div>;
}
