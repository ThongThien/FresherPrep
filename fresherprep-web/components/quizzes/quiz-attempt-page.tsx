...
export function QuizAttemptPage({ quizId, attemptId }: { quizId: string; attemptId: string }) {
  ...
  useEffect(() => {
    ...
    void fetch("/api/quiz-attempts/" + encodeURIComponent(attemptId), { signal: controller.signal, cache: "no-store" })
      .then(async (response) => {
        ...
        if (value.quizId !== quizId) return setError("This attempt does not belong to the requested quiz.");
        ...
        if (!(reason instanceof DOMException && reason.name === "AbortError")) setError("Unable to restore this attempt.");
      });
  ...
  async function submitAttempt() {
    ...
    try {
      ...
      if (value.status !== "SUBMITTED") {
        setError("The attempt was not completed. You can safely try again.");
        return;
      }
      router.replace("/quizzes/" + quizId + "/attempts/" + attempt.id + "/result");
    } catch { setError("Unable to submit this attempt. You can safely try again."); }
    finally {
  ...
  if (error === "NOT_FOUND") return <Message title="Attempt not found" message="This attempt does not exist or is not available to your account." />;
  if (!attempt && error) return <Message title="Attempt unavailable" message={error} retry={() => setReload((value) => value + 1)} />;
  if (!attempt || !question) return <div className="mx-auto max-w-5xl animate-pulse motion-reduce:animate-none" role="status"><span className="sr-only">Loading quiz attempt</span><div className="h-8 w-2/3 rounded bg-surface-strong" /><div className="mt-8 h-80 rounded-lg bg-surface" /></div>;

  return <div className="mx-auto max-w-5xl">
    <header className="border-b border-border pb-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-primary">Assessment in progress</p><h1 className="mt-2 text-2xl font-semibold text-text">{attempt.quizTitle}</h1></div><Badge>{answered} of {attempt.questions.length} answered</Badge></div><Progress className="mt-5" value={answered / attempt.questions.length * 100} label="Answer progress" showValue /></header>
    <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_15rem] lg:items-start">
      <main className="rounded-lg border border-border bg-surface p-5 shadow-card sm:p-7" aria-labelledby="question-title">
        <p className="text-sm font-semibold text-primary">Question {current + 1} of {attempt.questions.length}</p>
        ...
        {error ? <Feedback className="mt-5" tone="error" title="Request failed">{error}</Feedback> : null}
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between"><Button className="w-full sm:w-auto" variant="secondary" disabled={current === 0 || submitting} onClick={() => goToQuestion(current - 1)}>Previous</Button><Button className="w-full sm:w-auto" variant="secondary" disabled={current === attempt.questions.length - 1 || submitting} onClick={() => goToQuestion(current + 1)}>Next</Button></div>
      </main>
      <aside className="space-y-5 lg:sticky lg:top-24"><section className="rounded-lg border border-border bg-surface p-4"><h2 className="text-sm font-semibold text-text">Questions</h2><div className="mt-4 grid grid-cols-5 gap-2 lg:grid-cols-4">{attempt.questions.map((item, index) => { const itemAnswered = Boolean(answers[item.id]); return <button key={item.id} type="button" disabled={submitting} aria-label={"Question " + (index + 1) + (itemAnswered ? ", answered" : ", unanswered")} aria-current={index === current ? "step" : undefined} className={"min-h-10 rounded-md border text-sm font-semibold " + (index === current ? "border-primary-solid bg-primary-solid text-white" : itemAnswered ? "border-success/30 bg-success-subtle text-success-strong" : "border-border text-text-muted")} onClick={() => goToQuestion(index)}>{index + 1}<span className="sr-only">{itemAnswered ? " answered" : " unanswered"}</span></button>; })}</div><div className="mt-4 space-y-1 text-xs text-text-muted"><p>Answered: {answered}</p><p>Unanswered: {attempt.questions.length - answered}</p></div></section><Button className="w-full" variant="secondary" disabled={submitting} onClick={() => setConfirming(true)}>Submit quiz</Button></aside>
    </div>
    {confirming ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !submitting) setConfirming(false); }}><div className="w-full max-w-md rounded-lg bg-surface p-6 shadow-xl" role="dialog" aria-modal="true" aria-labelledby="submit-title" aria-describedby="submit-description"><h2 id="submit-title" className="text-xl font-semibold text-text">Submit this quiz?</h2><p id="submit-description" className="mt-3 text-sm leading-6 text-text-muted">{answered} answered and {attempt.questions.length - answered} unanswered. After submitting, answers can no longer be changed.</p><div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button ref={cancelButtonRef} variant="secondary" disabled={submitting} onClick={() => setConfirming(false)}>Cancel</Button><Button loading={submitting} onClick={() => void submitAttempt()}>{submitting ? "Submitting..." : "Confirm submit"}</Button></div></div></div> : null}
  </div>;
}

function Message({ title, message, retry }: { title: string; message: string; retry?: () => void }) {
  return <div className="mx-auto max-w-2xl py-10"><h1 className="text-2xl font-semibold text-text">{title}</h1><p className="mt-3 text-sm text-text-muted">{message}</p><div className="mt-5 flex gap-3">{retry ? <Button variant="secondary" onClick={retry}>Try again</Button> : null}<Link className="inline-flex min-h-10 items-center text-sm font-semibold text-primary" href="/learning-paths">Back to learning</Link></div></div>;
}
