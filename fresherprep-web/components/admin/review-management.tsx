...
export function ReviewManagement() {
  ...
  const columns: readonly AdminTableColumn<ContributionSummary>[] = [
    { key: "content", header: t("Content"), cell: (item) => <div><p className="font-semibold text-text">{item.title}</p><p className="mt-1 text-xs">{item.contentType}</p></div> },
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <section className="mt-7">
      <AdminToolbar>
        ...
        <AdminFilter label={t("Filter by content type")} value={type} onChange={(value) => { setType(value); setPage(0); }}>
          ...
          {(["LESSON", "QUESTION", "QUIZ"] satisfies ContributionContentType[]).map((value) => <option key={value}>{value}</option>)}
        </AdminFilter>
        <AdminFilter label={t("Filter by review status")} value={status} onChange={(value) => { setStatus(value as ReviewStatus); setPage(0); }}>
          {(["PENDING_REVIEW", "REJECTED", "PUBLISHED", "DRAFT"] satisfies ReviewStatus[]).map((value) => <option key={value}>{value}</option>)}
        </AdminFilter>
      </AdminToolbar>
      <div className="mt-5">
    ...
    {detail ? <section className="mt-8 border-t border-border pt-7" aria-labelledby="review-detail-title">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><h2 className="text-2xl font-semibold text-text" id="review-detail-title">{detail.submission.title}</h2><p className="mt-1 text-sm text-text-muted">{detail.submission.contentType} · {detail.submission.contributorName}</p></div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1.35fr_0.65fr]">
        <Card><CardContent>
          ...
          <pre className="mt-4 max-h-[32rem] overflow-auto whitespace-pre-wrap rounded-md bg-surface-muted p-4 text-xs leading-6 text-text">{JSON.stringify(detail.content, null, 2)}</pre>
        </CardContent></Card>
