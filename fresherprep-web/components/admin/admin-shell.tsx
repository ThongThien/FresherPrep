    ...
    <div className="min-h-dvh bg-background">
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-surface/95 backdrop-blur-sm">
        <div className="mx-auto flex h-16 max-w-[1600px] items-center px-4 sm:px-6 lg:px-8">
          <button type="button" onClick={() => setOpen(true)} className="mr-3 inline-flex size-11 items-center justify-center rounded-md text-text-muted hover:bg-surface-muted focus-visible:ring-3 focus-visible:ring-focus/20 lg:hidden" aria-label="Open admin navigation" aria-haspopup="dialog">
            <span aria-hidden="true" className="text-xl">☰</span>
          </button>
          <Link href="/admin" className="flex items-center gap-2.5 rounded-sm focus-visible:ring-3 focus-visible:ring-focus/20" aria-label="FresherPrep admin dashboard">
            ...
            <span className="rounded-sm border border-border bg-surface-muted px-2 py-0.5 text-xs font-semibold text-text-muted">Admin</span>
          </Link>
          {current ? <span className="ml-5 hidden border-l border-border pl-5 text-sm text-text-muted md:block">{current.label}</span> : null}
          <div className="ml-auto flex items-center gap-2">
            <ThemeToggle />
            <div className="hidden items-center gap-4 sm:flex">
              <Link href="/dashboard" className="inline-flex min-h-10 items-center text-sm font-semibold text-primary hover:text-primary-hover">Learning area</Link>
      ...
      <div className="mx-auto flex w-full max-w-[1600px]">
        <aside className="sticky top-16 hidden h-[calc(100dvh-4rem)] w-64 shrink-0 flex-col border-r border-border bg-surface lg:flex">
          <p className="px-5 pb-3 pt-6 text-xs font-semibold uppercase tracking-[0.12em] text-text-subtle">Management</p>
          <AdminNavigation className="flex-1 overflow-y-auto px-3 pb-6" />
          <div className="border-t border-border p-4">
            <Link href="/dashboard" className="inline-flex min-h-10 items-center text-sm font-semibold text-primary hover:text-primary-hover">Back to learning</Link>
    ...
    <dialog ref={dialogRef} onClose={onClose} onCancel={onClose} onClick={(event) => { if (event.target === event.currentTarget) onClose(); }} aria-labelledby="admin-navigation-title" className="m-0 h-dvh max-h-none w-full max-w-none bg-transparent p-0 backdrop:bg-text/35 lg:hidden">
      <aside className="flex h-full w-[min(21rem,88vw)] flex-col border-r border-border bg-surface shadow-card">
        <div className="flex h-16 items-center justify-between border-b border-border px-4">
          <div><p id="admin-navigation-title" className="text-sm font-semibold text-text">Admin workspace</p><p className="mt-0.5 text-xs text-text-muted">Manage FresherPrep content</p></div>
          <button type="button" onClick={onClose} autoFocus className="inline-flex size-11 items-center justify-center rounded-md text-text-muted hover:bg-surface-muted focus-visible:ring-3 focus-visible:ring-focus/20" aria-label="Close admin navigation">X</button>
        </div>
        <AdminNavigation onNavigate={onClose} className="flex-1 overflow-y-auto p-4" />
        <div className="border-t border-border p-4">
          <Link href="/dashboard" onClick={onClose} className="mb-3 inline-flex min-h-10 items-center text-sm font-semibold text-primary">Back to learning</Link>
