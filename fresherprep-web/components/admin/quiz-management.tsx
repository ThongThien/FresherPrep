...
function FixedQuestions({ quiz, questions, pending, mutate }: { quiz: QuizDetail; questions: Question[]; pending: boolean; mutate: (path: string, method: "POST" | "PUT" | "PATCH" | "DELETE", message: string, body?: unknown) => Promise<void> }) {
  ...
  const [position, setPosition] = useState(Math.max(0, ...quiz.fixedQuestions.map((item) => item.position)) + 1);
  ...
  return <Card><CardContent><h2 className="text-lg font-semibold text-text">{t("Fixed questions")}</h2><p className="mt-1 text-sm text-text-muted">{t("Positions must be unique. Backend has no separate reorder endpoint; remove and add with the intended position.")}</p>
    ...
    <div className="mt-6 grid gap-4 border-t border-border pt-6 sm:grid-cols-[1fr_8rem_auto]"><div><Label htmlFor="fixed-question">{t("Question")}</Label><Select id="fixed-question" value={questionId} disabled={quiz.status !== "DRAFT"} onChange={(event) => setQuestionId(event.target.value)}><option value="">{t("Select question")}</option>{available.map((question) => <option key={question.id} value={question.id}>{question.code} ({question.status})</option>)}</Select></div><div><Label htmlFor="fixed-position">{t("Position")}</Label><Input id="fixed-position" type="number" min={1} disabled={quiz.status !== "DRAFT"} value={position} onChange={(event) => setPosition(Number(event.target.value))} /></div><Button className="self-end" disabled={!questionId || quiz.status !== "DRAFT"} loading={pending} onClick={() => void mutate(`quizzes/${quiz.id}/fixed-questions`, "POST", t("Fixed question added."), { questionId, position })}>{t("Add")}</Button></div>
