...
export function LessonContent({ content }: { content: string }) {
  ...
  return <article className="lesson-prose">{blocks.map((block, index) => <ContentBlockView block={block} key={index} />)}</article>;
}
...
function ContentBlockView({ block }: { block: ContentBlock }) {
  ...
  if (block.type === "callout") return <aside className="lesson-callout" aria-label="Note">{inlineContent(block.text)}</aside>;
  ...
  return (
    <div className="overflow-hidden rounded-md border border-slate-700 bg-slate-950 text-slate-100">
      <div className="flex min-h-10 items-center justify-between gap-3 border-b border-slate-700 px-4">
        ...
        <button type="button" className="min-h-8 rounded-sm px-2 text-xs font-semibold text-slate-200 transition-colors hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/60" onClick={() => void copyCode()}>{copied ? "Copied" : "Copy"}</button>
      </div>
...
function parseContent(content: string): ContentBlock[] {
  ...
  return blocks.length ? blocks : [{ type: "paragraph", text: "Lesson content is not available yet." }];
}
