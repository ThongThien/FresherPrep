...
export function LearningProgressPage() {
  ...
  return <div className="mx-auto w-full max-w-6xl">
    <header className="border-b border-border pb-7"><p className="text-sm font-semibold text-primary">{t("Learning progress")}</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-text sm:text-4xl">{t("Your learning record")}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-text-muted">{t("A factual overview based on your joined paths, lesson activity, and quiz attempts.")}</p></header>

    <section className="mt-8 grid gap-4 sm:grid-cols-3" aria-label={t("Learning summary")}>
    ...
    <section className="mt-10" aria-labelledby="paths-title">
      <SectionHeading id="paths-title" title={t("Learning paths")} description={t("Progress percentages are calculated by the backend from required lessons and weights.")} />
      {data.issues.includes("pathProgress") ? <Feedback className="mt-4" tone="warning" title={t("Some path progress is unavailable")}><p>{t("Available paths remain listed below.")}</p><Button className="mt-3" size="sm" variant="secondary" onClick={() => setReload((value) => value + 1)}>{t("Retry progress")}</Button></Feedback> : null}
      {!data.pathEntries.length ? <Empty title={t("No joined learning paths")} message={t("Join a learning path to begin tracking structured progress.")} action="/learning-paths" /> : <div className="mt-5 grid gap-4 lg:grid-cols-2">{data.pathEntries.map((entry) => <article className="rounded-lg border border-border bg-surface p-5 shadow-card" key={entry.membership.id}><div className="flex items-start justify-between gap-4"><div><h3 className="font-semibold text-text">{entry.membership.learningPath.name}</h3><p className="mt-1 text-xs text-text-muted">{entry.membership.learningPath.technologyName}</p></div>{entry.progress?.completed ? <Badge variant="success">{t("Completed")}</Badge> : null}</div>{entry.progress ? <><Progress className="mt-5" value={entry.progress.progressPercentage} label={t("Required progress")} showValue tone={entry.progress.completed ? "success" : "primary"} /><p className="mt-3 text-xs text-text-muted">{t("{{completed}} of {{total}} required lessons completed", { completed: entry.progress.completedRequiredItems, total: entry.progress.requiredItems })}</p></> : <p className="mt-5 text-sm text-warning-strong">{t("Progress temporarily unavailable.")}</p>}<Link className="mt-4 inline-flex min-h-10 items-center text-sm font-semibold text-primary" href={"/learning-paths/" + entry.membership.learningPath.id}>{t("View path")}</Link></article>)}</div>}
    </section>

    <section className="mt-10" aria-labelledby="lessons-title">
      <SectionHeading id="lessons-title" title={t("Lesson progress")} description={t("Recent reading activity and backend-confirmed completion.")} />
      {data.issues.includes("lessons") ? <SectionError onRetry={() => setReload((value) => value + 1)} /> : !data.lessons.content.length ? <Empty title={t("No lesson activity")} message={t("Open a lesson to start recording reading progress.")} action="/learning-paths" /> : <div className="mt-5 overflow-hidden rounded-lg border border-border bg-surface">{data.lessons.content.slice(0, 10).map((lesson) => <LessonRow key={lesson.lessonId} lesson={lesson} />)}</div>}
      {activeLessons.length || completedLessons.length ? <p className="mt-3 text-xs text-text-muted">{t("{{active}} in progress · {{completed}} completed", { active: activeLessons.length, completed: completedLessons.length })}</p> : null}
    </section>

    <section className="mt-10" id="quiz-activity" aria-labelledby="quiz-title">
      <SectionHeading id="quiz-title" title={t("Quiz activity")} description={t("Recent attempts and server-calculated results.")} />
      {data.issues.includes("quizAttempts") ? <SectionError onRetry={() => setReload((value) => value + 1)} /> : !data.quizAttempts.content.length ? <Empty title={t("No quiz attempts")} message={t("Quiz attempts will appear here after you begin an assessment.")} /> : <div className="mt-5 overflow-hidden rounded-lg border border-border bg-surface">{data.quizAttempts.content.slice(0, 10).map((attempt) => <AttemptRow key={attempt.id} attempt={attempt} />)}</div>}
    </section>
  </div>;
}
...
function LessonRow({ lesson }: { lesson: LessonProgress }) {
  ...
  return <div className="flex flex-col gap-3 border-b border-border p-4 last:border-b-0 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-text">{lesson.lessonTitle}</p><p className="mt-1 text-xs text-text-muted">{t("Last viewed {{date}} · Scroll {{percent}}%", { date: formatDate(lesson.lastViewedAt, locale), percent: lesson.maxScrollPercent })}</p></div><div className="flex items-center gap-3"><Badge variant={lesson.completed ? "success" : lesson.readQualified ? "info" : "neutral"}>{lesson.completed ? t("Completed") : lesson.readQualified ? t("Reading qualified") : t("In progress")}</Badge><Link className="text-sm font-semibold text-primary" href={"/lessons/" + lesson.lessonId}>{lesson.completed ? t("Review") : t("Continue")}</Link></div></div>;
}
...
function AttemptRow({ attempt }: { attempt: QuizAttemptSummary }) {
  ...
  return <div className="flex flex-col gap-3 border-b border-border p-4 last:border-b-0 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-text">{attempt.quizTitle}</p><p className="mt-1 text-xs text-text-muted">{formatDate(attempt.submittedAt ?? attempt.startedAt, locale)}</p></div><div className="flex flex-wrap items-center gap-3"><Badge variant={!submitted ? "info" : attempt.passed ? "success" : "danger"}>{!submitted ? t("In progress") : attempt.passed ? t("Passed") : t("Not passed")}</Badge>{submitted && attempt.scorePercentage !== null ? <span className="text-sm font-semibold tabular-nums text-text">{formatScore(attempt.scorePercentage)}</span> : null}<Link className="text-sm font-semibold text-primary" href={href}>{submitted ? t("View result") : t("Resume")}</Link></div></div>;
}
