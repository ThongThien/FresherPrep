...
export async function GET(request: NextRequest, context: { params: Promise<{ attemptId: string }> }) {
  ...
  try {
    const result = await authenticatedBackendRequest(request, "/api/quiz-attempts/" + encodeURIComponent(attemptId));
    ...
    return apiError(503, "QUIZ_SERVICE_UNAVAILABLE", "Unable to load this attempt.");
  }
}
